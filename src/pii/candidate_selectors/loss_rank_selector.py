    ...
    for i in tqdm(range(0, len(inter_dataset_slice), infer_batch_size), desc="Computing loss"):
        ...
        with torch.no_grad():
            ...
            for bi in range(bs):
                ...
                # sum(log(ranks + 1))
                rank_score = np.sum(np.log(current_ranks.cpu().float().numpy() + 1))
...
def map_ll_to_mean(example, ignore_pre_pii=True):
    ...
    return {'mean_logits': mean_logit, 'mean_loss': mean_loss, 'mean_rank': mean_rank}


class LossRankSelector(AbstractCandidateSelector):
    ...
    def select_candidates(
        ...
        refer_inter_dataset: Optional[Dataset] = None,
        refer_bias: float = 0.,
        ...
        if self.refer_model_name_or_path and not refer_inter_dataset:
            # prepare args
            args_for_devices = []
            for idx in range(device_parallel_size):
                inter_dataset_slice = dataset_slices[idx]
        ...
        if refer_inter_dataset:
            ...
            processed_inter_dataset = processed_inter_dataset.map(lambda x: {
                'mean_logits': x['mean_logits'] - x['refer_mean_logits'], 
                'mean_loss': (x['mean_loss'] - x['refer_mean_loss'] - refer_bias) / (x['refer_mean_loss'] + refer_bias)
            }, num_proc=8)
        processed_inter_dataset = processed_inter_dataset.map(lambda x: {
            "final_score": (1 - rank_alpha)*x['mean_loss'] + rank_alpha*x['mean_rank']
        }, num_proc=8)

        inter_df = processed_inter_dataset.to_pandas()
        inter_df = inter_df.groupby(["row_idx", "pii_key", "candidate"])[['mean_logits', 'mean_loss', 'mean_rank', 'final_score']].sum().reset_index() # sum masked_seq
        ...
        def add_scores(example):
            ...
            for candidate in candidates:
                ...
                final_score = candidate_df["final_score"].values[0]
                loss_scores.append(loss_score)
                logits_scores.append(logits_score)
                rank_scores.append(rank_score)
                final_scores.append(final_score)
            return {'scores': final_scores, 'logits_scores': logits_scores, 'loss_scores': loss_scores, 'rank_scores': rank_scores}
