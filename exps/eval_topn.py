...
def main():
    ...
    #################### eval ####################
    candidates_with_scores = load_from_disk(eval_args.selected_data)
    accuracy = top_one_accuracy(dataset, candidates_with_scores)
    print(f"Top 1 accuracy: {accuracy}")

    top_2_accuracy = top_n_accuracy(dataset, candidates_with_scores, n=2)
    print(f"Top 2 accuracy: {top_2_accuracy}")

    top_3_accuracy = top_n_accuracy(dataset, candidates_with_scores, n=3)
    print(f"Top 3 accuracy: {top_3_accuracy}")

    top_5_accuracy = top_n_accuracy(dataset, candidates_with_scores, n=5)
    print(f"Top 5 accuracy: {top_5_accuracy}")

    top_10_accuracy = top_n_accuracy(dataset, candidates_with_scores, n=10)
    print(f"Top 10 accuracy: {top_10_accuracy}")

    top_all_accuracy = top_n_accuracy(dataset, candidates_with_scores, n=-1)
